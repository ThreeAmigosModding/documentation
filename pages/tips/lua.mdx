...
Don't do `GetHashKey('WEAPON_PISTOL')`, do ``GethashKey(`WEAPON_PISTOL`)`` instead. `GetHashKey('WEAPON_PISTOL')` converts a string into a hash each time the function is called. Using backticks will instead convert the string into hash during compilation, so any checks will have it hashed in the first place.
